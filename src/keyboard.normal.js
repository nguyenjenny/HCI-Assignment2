...
class KeyboardNormal extends React.Component {
  ...
  _onTouchEnd(e) {
    ...
    if (this.justGestured === true) {
      ...
      var x =
        (touch.clientX - this.offsetLeft) /
        (this.position.width / this.state.originalDimensions.width);
      var y =
        (touch.clientY - this.offsetTop) /
        (this.position.height / this.state.originalDimensions.height);
      e.nativeEvent.preventDefault();
      e.nativeEvent.stopPropagation();
  ...
  onLoad({ target: img }) {
    ...
    if (this.displaySize !== undefined) {
      this.config.originalScale =
        this.displaySize.width / this.originalDimensions.width;
      /*
			this.setState({
  ...
  onMouseDown(e) {
    ...
    console.log(
      "[onMouseDown] position: " + this.position.x + ":" + this.position.y
    );
    this.onKeyClick({ x: x, y: y });
    e.preventDefault();
    e.stopPropagation();
  ...
  getKeyChar = pt => {
    ...
    var keys =
      this.state.keyboardImg === this.imgs[0] ? Keymaps.keys : Keymaps.keys_sym;
    ...
    for (var i = 0, len = keys.length; i < len; i++) {
      ...
      if (
        keychar.x <= pt.x &&
        keychar.y <= pt.y &&
        keychar.x + keychar.width >= pt.x &&
        keychar.y + keychar.height >= pt.y
      ) {
        ...
        if (
          (minDistanceKey === null || dSquared < minDistance) &&
          dSquared <
            maxKeyErrorDistSquared *
              Math.pow(Math.min(keychar.width, keychar.height), 2)
        ) {
